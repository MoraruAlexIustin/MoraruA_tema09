...
namespace OpenTK_winforms_z02
{
    public partial class Form1 : Form
    {
        ...
        private bool statusPyramidTex; // NOU: Stare pentru desenarea piramidei texturate
        ...
        // Stări de control obiecte 3D (Texturate).
        ...
        // NOU: Vertexurile si lista de indici pentru o Piramidă cu Baza Pătrată (definire în cod).
        private readonly int[,] pyramidVertices = new int[,] {
            { -20, 0, -20 }, // V0: Baza stanga-fata
            { 20, 0, -20 },  // V1: Baza dreapta-fata
            { 20, 0, 20 },   // V2: Baza dreapta-spate
            { -20, 0, 20 },  // V3: Baza stanga-spate
            { 0, 40, 0 }     // V4: Vârf
        };

        private readonly int[] pyramidTrianglesList = new int[] {
            // Fețe laterale (4 triunghiuri)
            0, 1, 4, // Fața din față (V0, V1, V4)
            1, 2, 4, // Fața din dreapta (V1, V2, V4)
            2, 3, 4, // Fața din spate (V2, V3, V4)
            3, 0, 4, // Fața din stanga (V3, V0, V4)
            // Bază (2 triunghiuri)
            0, 2, 3, // Triunghi 1 (V0, V2, V3)
            0, 1, 2  // Triunghi 2 (V0, V1, V2)
        };
        // END NOU
        ...
        //# SET 1 (Parametrii pentru Sursa 0 de lumină)
        ...
        //-----------------------------------------------------------------------------------------
        //-----------------------------------------------------------------------------------------
        //   CONTROL CAMERĂ (Metodele nu sunt complete fără controalele UI corespunzătoare, dar le păstrez pentru compatibilitate)
        ...
        //-----------------------------------------------------------------------------------------
        //-----------------------------------------------------------------------------------------
        //   CONTROL ILUMINARE (Metodele nu sunt complete fără controalele UI corespunzătoare, dar le păstrez pentru compatibilitate)
        ...
        private void GlControl1_Paint(object sender, PaintEventArgs e)
        {
            ...
            Matrix4 perspective = Matrix4.CreatePerspectiveFieldOfView((float)camDepth, 4 / 3f, 1, 10000); // 4/3f pentru float division
            ...
            // Controlul rotației cu mouse-ul.
            if (statusControlMouse2D == true)
            {
            ...
            // Desenăm piramida texturată.
            if (statusPyramidTex == true)
            {
        ...
        {
            if (textures[0] == 0)
            { // Generează texturile doar la prima apelare
        ...
        {
            // Se asigură că fișierul există înainte de a încerca să încarce
            if (!File.Exists(filename))
            {
                Console.WriteLine($"Eroare: Fișierul de textură '{filename}' nu a fost găsit.");
                return;
            }
        ...
        // NOU: Metodă pentru activarea desenării piramidei texturate
        public void btnPyramidTex_Click(object sender, EventArgs e)
        {
            resetTexCubes();
            setCubeStatus("OFF");
            statusPyramidTex = true;
            GlControl1.Invalidate();
        }
        // END NOU
        ...
        private void resetTexCubes()
        {
            ...
            statusCubeTex3 = false;
            statusCubeTex4 = false;
            statusPyramidTex = false; // NOU: Resetare stare piramidă
            GlControl1.Invalidate();
        }
        ...
        // Desenează piramida texturată - Demonstrarea texturării pe o suprafață triunghiulară.
        private void DeseneazaPiramida_Tex1()
        {
            ...
            // Culoarea albă permite maparea texturii fără alterarea culorii originale.
            switch (colorTex)
            {
                case 0: GL.Color3(Color.White); break;
                case 1: GL.Color3(Color.FromArgb(0, 255, 0, 0)); break;
            ...
            // Fețele laterale (4 triunghiuri)
            for (int i = 0; i < 4 * 3; i += 3)
            {
                // Vârful V4: mapat la mijlocul sus (0.5, 1.0) - Aici apare deformarea
                GL.TexCoord2(0.5, 1.0);
                int v_varf = pyramidTrianglesList[i + 2]; // V4
                GL.Vertex3(pyramidVertices[v_varf, 0], pyramidVertices[v_varf, 1], pyramidVertices[v_varf, 2]);

                // Un punct de bază: mapat la stânga jos (0.0, 0.0)
                GL.TexCoord2(0.0, 0.0);
                int v_baza1 = pyramidTrianglesList[i];
                GL.Vertex3(pyramidVertices[v_baza1, 0], pyramidVertices[v_baza1, 1], pyramidVertices[v_baza1, 2]);

                // Celălalt punct de bază: mapat la dreapta jos (1.0, 0.0)
                GL.TexCoord2(1.0, 0.0);
                int v_baza2 = pyramidTrianglesList[i + 1];
                GL.Vertex3(pyramidVertices[v_baza2, 0], pyramidVertices[v_baza2, 1], pyramidVertices[v_baza2, 2]);
            }

            // Baza (2 triunghiuri) - mapare normală 2D pe un pătrat
            int i_base = 4 * 3;

            // Baza - Triunghiul 1 (V0, V2, V3)
            GL.TexCoord2(0.0, 0.0); GL.Vertex3(pyramidVertices[pyramidTrianglesList[i_base], 0], pyramidVertices[pyramidTrianglesList[i_base], 1], pyramidVertices[pyramidTrianglesList[i_base], 2]); // V0
            GL.TexCoord2(1.0, 1.0); GL.Vertex3(pyramidVertices[pyramidTrianglesList[i_base + 1], 0], pyramidVertices[pyramidTrianglesList[i_base + 1], 1], pyramidVertices[pyramidTrianglesList[i_base + 1], 2]); // V2
            GL.TexCoord2(0.0, 1.0); GL.Vertex3(pyramidVertices[pyramidTrianglesList[i_base + 2], 0], pyramidVertices[pyramidTrianglesList[i_base + 2], 1], pyramidVertices[pyramidTrianglesList[i_base + 2], 2]); // V3

            // Baza - Triunghiul 2 (V0, V1, V2)
            GL.TexCoord2(0.0, 0.0); GL.Vertex3(pyramidVertices[pyramidTrianglesList[i_base + 3], 0], pyramidVertices[pyramidTrianglesList[i_base + 3], 1], pyramidVertices[pyramidTrianglesList[i_base + 3], 2]); // V0
            GL.TexCoord2(1.0, 0.0); GL.Vertex3(pyramidVertices[pyramidTrianglesList[i_base + 4], 0], pyramidVertices[pyramidTrianglesList[i_base + 4], 1], pyramidVertices[pyramidTrianglesList[i_base + 4], 2]); // V1
            GL.TexCoord2(1.0, 1.0); GL.Vertex3(pyramidVertices[pyramidTrianglesList[i_base + 5], 0], pyramidVertices[pyramidTrianglesList[i_base + 5], 1], pyramidVertices[pyramidTrianglesList[i_base + 5], 2]); // V2
